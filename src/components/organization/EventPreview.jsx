...
    const startDate = eventData.date_Start || eventData.startDate;
    const endDate = eventData.date_End || eventData.endDate;
    
...
  // --- START: Logic điều chỉnh ở đây ---
  const displayPrice = (() => {
    // Nếu có các loại vé và hasTickets là true, hiển thị khoảng giá của vé
    if (eventData.hasTickets && eventData.ticketTypes?.length > 0) {
      const minPrice = Math.min(...eventData.ticketTypes.map(ticket => ticket.price || 0));
      const maxPrice = Math.max(...eventData.ticketTypes.map(ticket => ticket.price || 0));
      
...
      }
    } 
    // Nếu không có loại vé hoặc hasTickets là false, hiển thị giá từ trường 'price'
    else if (eventData.price !== undefined && eventData.price !== null) {
      return eventData.price === 0 ? 'Miễn phí' : `${eventData.price.toLocaleString('vi-VN')} VNĐ`;
    }
    return 'Miễn phí'; // Giá trị mặc định nếu không có thông tin
  })();
  // --- END: Logic điều chỉnh ở đây ---
...
            </div>
            
            <Title level={2}>{eventData.name || eventData.title}</Title>
            
...
                        <br />
                        <Text>{formatDate(eventData.date_Start || eventData.startDate)}</Text>
                      </div>
                    ),
                  },
                  {
...
                        <br />
                        <Text>{formatDate(eventData.date_End || eventData.endDate)}</Text>
                      </div>
                    ),
                  },
                ]}
...
                <Text>Sức chứa: <Text strong>{eventData.slot || eventData.maxAttendees} người</Text></Text>
              </div>
              
              <Divider style={{ margin: '12px 0' }} />
              
...
                    <TagsOutlined style={{ color: '#722ed1', marginRight: 8 }} />
                    <Text>Danh mục: <Text strong>{eventData.cateID}</Text></Text>
                  </div>
                </>
              )}
            </Card>
